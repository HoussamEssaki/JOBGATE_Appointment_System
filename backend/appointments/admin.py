# FileName: MultipleFiles/admin.py (appointments app)
...
# No longer need to import UniversityStaff from universities here, as staff is now User
...
class AgendaStaffAssignmentInline(admin.TabularInline):
    ...
    raw_id_fields = ('staff',) # Add this as staff is now a User
...
class AgendaAdmin(admin.ModelAdmin):
    ...
    # Adjusted search_fields for created_by (now a User) and university (now UniversityProfile)
    search_fields = ('name', 'description', 'university__display_name', 'created_by__first_name', 'created_by__last_name')
    readonly_fields = ('created_at', 'updated_at')
    ...
    def get_queryset(self, request):
        # Adjusted select_related for created_by (now a User)
        return super().get_queryset(request).select_related(
            'university', 'theme', 'created_by'
        )
...
class CalendarSlotAdmin(admin.ModelAdmin):
    ...
    # Adjusted search_fields for staff (now a User)
    search_fields = ('agenda__name', 'staff__first_name', 'staff__last_name', 'location')
    readonly_fields = ('created_at', 'updated_at')
    ...
    def staff_name(self, obj):
        # Adjusted to access User fields directly
    ...
    def get_queryset(self, request):
        # Adjusted select_related for staff (now a User)
        return super().get_queryset(request).select_related(
            'agenda', 'staff'
        )
...
class AppointmentAttachmentInline(admin.TabularInline):
    ...
    raw_id_fields = ('uploaded_by',) # Add this as uploaded_by is a User
...
class AppointmentStatisticsAdmin(admin.ModelAdmin):
    ...
    # Adjusted search_fields for staff (now a User) and university (now UniversityProfile)
    search_fields = ('university__display_name', 'theme__name', 'staff__first_name', 'staff__last_name')
    readonly_fields = ('created_at', 'updated_at')
    date_hierarchy = 'date'
...
class AgendaStaffAssignmentAdmin(admin.ModelAdmin):
    ...
    # Adjusted search_fields for staff (now a User)
    search_fields = ('agenda__name', 'staff__first_name', 'staff__last_name')
