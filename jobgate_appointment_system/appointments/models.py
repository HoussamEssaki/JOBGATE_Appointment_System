...
class Agenda(models.Model):
    university = models.ForeignKey('universities.University', on_delete=models.CASCADE, related_name='agendas')
    created_by = models.ForeignKey('universities.UniversityStaff', on_delete=models.CASCADE, related_name='created_agendas')
...
class CalendarSlot(models.Model):
    ...
    staff = models.ForeignKey('universities.UniversityStaff', on_delete=models.CASCADE, related_name='calendar_slots')
...
class Appointment(models.Model):
    ...
    talent = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='appointments')
...
class AppointmentStatistics(models.Model):
    university = models.ForeignKey('universities.University', on_delete=models.CASCADE, related_name='statistics')
    theme = models.ForeignKey(AppointmentTheme, on_delete=models.CASCADE, related_name='statistics')
    staff = models.ForeignKey('universities.UniversityStaff', on_delete=models.SET_NULL, null=True, blank=True, related_name='statistics')
    ...
    def __str__(self):
        return f"Stats for {self.university.name} - {self.date}"
...
class AgendaStaffAssignment(models.Model):
    ...
    staff = models.ForeignKey('universities.UniversityStaff', on_delete=models.CASCADE, related_name='agenda_assignments')
    role = models.CharField(max_length=50, choices=ROLE_CHOICES, default='advisor')
    is_primary = models.BooleanField(default=False)
    created_at = models.DateTimeField(default=timezone.now)
    
    def __str__(self):
        return f"{self.staff.user.first_name} {self.staff.user.last_name} - {self.agenda.name}"
