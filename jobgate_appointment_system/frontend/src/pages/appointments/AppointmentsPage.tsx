...
export const AppointmentsPage: React.FC = () => {
  ...
  const [, setAppointmentsError] = useState<string | null>(null);
  ...
  const fetchAppointments = async () => {
      ...
      try {
        ...
        params.append('page_size', '12'); // Assuming 12 items per page for pagination

        const response = await apiMethods.get<{ results: Appointment[]; count: number }>(`/appointments/?${params.toString()}`);
        setAppointments(response.results);
        setTotalCount(response.count);
      } catch (err: any) {
        setAppointmentsError(err.response?.data?.detail || 'Failed to fetch appointments.');
        console.error(err);
      } finally {
