...
export const ManageSlotsPage: React.FC = () => {
  ...
  const fetchSlots = async () => {
      ...
      setError(null);
      try {
        const slotsResponse = await apiMethods.get<CalendarSlot[] | { results: CalendarSlot[] }>('appointments/slots/');
        setSlots(Array.isArray(slotsResponse) ? slotsResponse : (slotsResponse as any).results || []);

        const agendasResponse = await apiMethods.get<Agenda[] | { results: Agenda[] }>('appointments/agendas/');
        setAgendas(Array.isArray(agendasResponse) ? agendasResponse : (agendasResponse as any).results || []);
  ...
  const handleStatusChange = async (newStatus: string) => {
      ...
      setError(null);
      try {
        await apiMethods.patch(`appointments/slots/${selectedSlot.id}/`, { status: newStatus });
        handleMenuClose();
        fetchSlots(); // Refresh data
      } catch (err: any) {
        setError(err.response?.data?.detail || `Failed to update slot status to ${newStatus}.`);
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = () => {
    ...
    const confirmDelete = async () => {
      ...
      setError(null);
      try {
        await apiMethods.delete(`appointments/slots/${selectedSlot.id}/`);
