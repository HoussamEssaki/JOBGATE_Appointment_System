...
class CalendarSlotCreateSerializer(serializers.ModelSerializer):
    ...
    # 1) Field-level validation (catches 0 or non-existent agendas early)
    def validate_agenda_id(self, value):
        if value == 0 or not Agenda.objects.filter(pk=value).exists():
            raise serializers.ValidationError("Invalid agenda selected.")
        return value
